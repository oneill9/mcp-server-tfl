/**
 * Test-only fixture adapted and modified from the MCP TypeScript SDK v2
 * conformance server at revision 3c7ddafa05d8f17fb52168bf4638f09251c3d0ff:
 * https://github.com/modelcontextprotocol/typescript-sdk/blob/3c7ddafa05d8f17fb52168bf4638f09251c3d0ff/test/conformance/src/everythingServer.ts
 * Upstream licensing and attribution: ../../../THIRD_PARTY_NOTICES.md
 *
 * It composes the production TfL registrations with diagnostic tools,
 * resources, prompts, and flows required by the referee's hard-coded probes,
 * then exercises them through this project's production HTTP adapter. The
 * diagnostic registrations are never selected by the production entry point.
 *
...
import { registerTflTools } from '../../src/index.js';
...
let notifyPromptsChanged: () => void = () => {};

export function configureConformanceNotifications(notify: {
    toolsChanged: () => void;
    promptsChanged: () => void;
}): void {
    notifyToolsChanged = notify.toolsChanged;
    notifyPromptsChanged = notify.promptsChanged;
}
...
export function createConformanceServer() {
    ...
    mcpServer.registerTool(
        ...
        async (_args, ctx): Promise<CallToolResult | InputRequiredResult> => {
            ...
            const declared = (ctx.mcpReq.envelope as Record<string, unknown> | undefined)?.[
                CLIENT_CAPABILITIES_META_KEY

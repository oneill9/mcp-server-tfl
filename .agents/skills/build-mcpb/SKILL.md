...
1. **Run the build script** — execute `bash scripts/build-mcpb.sh` from the project root. This will:
   ...
   - Pack the extension into `build/tfl-mcp-server.mcpb` using `mcpb pack`

2. **Report the result** — tell the user the path to the built `.mcpb` file and remind them they can install it with `mcpb install build/tfl-mcp-server.mcpb` or double-click it to open in Claude Desktop.

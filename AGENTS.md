...
The server has one Node.js implementation that exposes the same 6 tools over both supported transports.

- **Language:** TypeScript (compiled to ESM)
- **Runtime:** Node.js 22
- **Build:** `tsc` via npm scripts
- **MCP SDK:** `@modelcontextprotocol/server` 2.x and `@modelcontextprotocol/node` 2.x
- **MCP protocol:** `2026-07-28` for modern stdio and HTTP; stdio also serves 2025-era desktop clients, while HTTP rejects legacy requests
- **MCP transports:** stdio and stateless Streamable HTTP at `/mcp`
- **Entry point:** `node/src/index.ts` → `node/dist/index.js`
- **HTTP entry point:** `node/src/http.ts`, selected with `--http`
- **Tests:** Vitest unit/acceptance tests, official MCP conformance, and live TfL contract tests
- **MCPB manifest:** `node/manifest.json`
- **OCI image:** Node.js 22; stdio by default and Streamable HTTP with `--http`
...
- Uses an appropriate type and optional scope, such as `feat(node):`, `fix(node):`, `test:`, `docs:`, `refactor:`, or `chore:`
- Describes the user-visible or maintainer-visible change in the imperative mood
- Mentions the issue number when the work is tied to one, for example `feat(node): add line status filters (#123)`, or in the commit body
...
- `feat(node): expose service status MCP app resource`
- `refactor(node): load MCP app HTML from shared resources`
- `test(node): cover MCP app resource metadata (#123)`
...
1. **Write tests**: Add a failing acceptance test in `node/test/server.test.ts` and transport coverage in `node/test/http.test.ts` where relevant.
2. **Implement in Node.js**: Add the schema and tool implementation in `node/src/index.ts`.
3. **Add annotations**: Ensure the tool has either `readOnlyHint: true` or `destructiveHint: true`.
4. **Update docs**: Add the tool description and parameters to `docs/tools.md`.
5. **Verify protocol behavior**: Run unit/HTTP tests and the conformance suite before moving on.

### Continuous Integration & PR Rules

- **Tests must pass before push/PR**: Run `cd node && npm ci && npm run build && npm test && npm run test:conformance`. Run `npm run contractTest` when live-network verification is in scope.
- **Branching**: Commit against your designated feature branch (e.g., `claude/tdd-feature...`). Do not force-push to `main`.

### Running & Debugging Locally

- **stdio**: Navigate to `node/`, run `npm run build`, and start `node dist/index.js`.
- **Streamable HTTP**: Run `HOST=127.0.0.1 PORT=8080 node dist/index.js --http` and connect to `http://127.0.0.1:8080/mcp`.
...
- Production dependencies must remain on the modular MCP SDK 2.x packages. The v1 `@modelcontextprotocol/sdk` package is permitted only as a transitive development dependency of the official conformance runner until that runner migrates.
- Do not reintroduce the retired Java implementation or legacy HTTP/SSE transport.
...
All tools must include `readOnlyHint` or `destructiveHint`.
...
The privacy policy must remain available for MCPB/Desktop extension submission.
...
- [x] Test the server over stdio and verify all 6 tools end-to-end — covered by `node/test/contract.test.ts`
- [x] Test the server over stateless Streamable HTTP — covered by `node/test/http.test.ts`
- [x] Pass the official frozen MCP `2026-07-28` requirement set with no expected-failure baseline; optional extension scenarios outside that set are not release criteria
- [x] Ensure Node unit and HTTP tests pass: `cd node && npm test`
- [x] Ensure live TfL contract tests pass: `cd node && npm run contractTest`
